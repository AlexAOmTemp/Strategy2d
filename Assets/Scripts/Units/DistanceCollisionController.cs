...
public class DistanceCollisionController : MonoBehaviour
{
    ...
    public GameObject AddDistanceDetector(string name, float range, DistanceCollisionDetector.TriggerEnter triggerEnter,
        DistanceCollisionDetector.TriggerExit triggerExit)
    {
        var detector = Instantiate(_distanceDetectorPrefab, this.transform.position, Quaternion.identity,
            this.transform);
    ...
    public void onDistance(GameObject sender, Collider2D collider)
    {
        ...
        Debug.Log(
            $"DistanceCollisionController: Game Object {collider.name} on the {sender.name} distance of unit {gameObject.name}");
    }
    ...
    public void onDistanceExit(GameObject sender, Collider2D collider)
    {
        ...
        Debug.Log(
            $"DistanceCollisionController: Game Object {collider.name} has left the {sender.name} distance of unit {gameObject.name}");
    }
}