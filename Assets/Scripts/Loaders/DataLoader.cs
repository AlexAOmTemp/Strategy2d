...
public class DataLoader : MonoBehaviour
{
    ...
    private void buildingParser(string xml)
    {
        ...
        if (xRoot != null)
        {
            ...
            foreach (XmlElement building in xRoot)
            {
                ...
                buildingData =
                    (BuildingData) (loadParameters(unitDataObj, building.ChildNodes, _buildingsSpritesFolder));
        ...
        if (xRoot != null)
        {
            ...
            foreach (XmlElement level in xRoot)
            {
                ...
                foreach (XmlNode levelParameter in level.ChildNodes)
                {
                    ...
                    {
                        Sprite sprite = (_dataFilesLoader.LoadedSpriteFiles[_levelsSpritesFolder]
                            .Find(i => i.name == levelParameter.InnerText));
                        if (sprite == null)
                        ...
                        else
                        {
                            ...
                            levelData.Background = Sprite.Create(texture,
                                new Rect(0.0f, 0.0f, texture.width, texture.height), new Vector2(0.5f, 0.5f), 100.0f, 0,
                    ...
                    {
                        levelData.SeparatorCommandBehindWallIcon =
                            findSprite(_levelsSpritesFolder, levelParameter.InnerText);
                    }

                    if (levelParameter.Name == "commandProtectWallIcon")
                    {
                        levelData.SeparatorCommandProtectWallIcon =
                            findSprite(_levelsSpritesFolder, levelParameter.InnerText);
                    }

                    if (levelParameter.Name == "commandForwardIcon")
                    {
                        levelData.SeparatorCommandForwardIcon =
                            findSprite(_levelsSpritesFolder, levelParameter.InnerText);
                    }
                        ...
                        foreach (XmlNode zoneParameter in levelParameter.ChildNodes)
                        {
                            if (zoneParameter.Name == "sizeInTitles")
                            {
                                ...
                                if (zoneData.SizeInTiles < 0 || zoneData.SizeInTiles > 1000)
                                    Debug.LogError(
                                        $"DataLoader: levelParser incorrect zone {zoneData.Name}:{zoneData.Id} size value = {zoneData.SizeInTiles}");
                            }
                            ...
                            if (zoneParameter.Name == "groundSprite")
                            {
                                ...
                                if (zoneData.GroundSprite == null)
                                    Debug.LogError(
                                        $"DataLoader: Sprite {_levelsSpritesFolder}{zoneParameter.InnerText} doesn't exist");
                            }

                            if (zoneParameter.Name == "separatorSprite")
                            {
                                Sprite sprite = (_dataFilesLoader.LoadedSpriteFiles[_levelsSpritesFolder]
                                    .Find(i => i.name == zoneParameter.InnerText));
                                if (sprite == null)
                                    Debug.LogError(
                                        $"DataLoader: Sprite {_levelsSpritesFolder}/{zoneParameter.InnerText} doesn't exist");
                                else
    ...
    private void consoleLogLoadedLevelData()
    {
        ...
        {
            Debug.Log(
                $"Name = {level.Name} Id = {level.Id} GroundLevel = {level.GroundLevel} DrowningInGround = {level.DrowningInGround} Zones = {level.ZonesCount}");
            foreach (ZoneData zData in level.Zones)
                Debug.Log(
                    $"zoneData.id = {zData.Id} zoneData.name = {zData.Name} zoneData.size = {zData.SizeInTiles} zoneData.Sprite = {zData.GroundSprite} zoneData.SeparatorSprite  {zData.SeparatorSprite}");
        }
    }
    ...
    {
        Sprite sprite = (_dataFilesLoader.LoadedSpriteFiles[folderName]
            .Find(i => i.name.ToLower() == spriteName.ToLower()));
        if (sprite == null)
