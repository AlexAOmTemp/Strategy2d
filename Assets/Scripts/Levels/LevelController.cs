...
public class LevelController : MonoBehaviour
{
    ...
    private void backgroundSet()
    {
        ...
        backgroundSpriteRenderer.size =
            new Vector2((Math.Abs(_finishPosition.position.x - _startPosition.position.x)) + 50, 12.64f);
    ...
    private void createZone(ZoneData zone, ref Vector3 position)
    {
        ...
        _zoneSeparators.Add(instantiateSeparator(new Vector3(position.x, PlacementLevel, 0),
            zoneHierarchyFolder.transform, zone, _groundSize));
        ...
        if (zone.Background != null)
        {
            ...
            backgroundRenderer.size =
                new Vector2((Math.Abs(startBackGroundPosition - position.x)), backgroundRenderer.size.y);
            ...
            if (Reverse == false)
                backgroundPositionX =
                    startBackGroundPosition + (position.x - startBackGroundPosition) / 2 - _groundSize.x;
            else
                backgroundPositionX =
                    startBackGroundPosition + (position.x - startBackGroundPosition) / 2 + _groundSize.x;
    ...
    {
        var blockerStart = Instantiate(_blockerPrefab, new Vector3(_startPosition.position.x, PlacementLevel, 0),
            Quaternion.identity, this.transform);
        var blockerFinish = Instantiate(_blockerPrefab, new Vector3(_finishPosition.position.x, PlacementLevel, 0),
            Quaternion.identity, this.transform);
        if (Reverse == false)
        {
